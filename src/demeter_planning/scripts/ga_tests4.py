...
# main constraints
for j in I:
    mdl.addConstraint(pulp.lpSum(x[i, j] for i, j_ in x_keys if j_ == j) == 1, name='C1_{}'.format(j))

for k in I:
    mdl.addConstraint(pulp.lpSum(x[i, k] for i, j in x_keys if j == k) ==
                      pulp.lpSum(x[k, j] for i, j in x_keys if i == k), name='C2_{}'.format(k))
...
if status == 'Optimal':
    print('Optimal solution found!'.format())
    ...
    print('Model is not optimal. Status: {}'.format(status))
# endregion